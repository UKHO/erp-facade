...
namespace UKHO.ERPFacade.EventAggregation.WebJob.Services
{
    public class AggregationService : IAggregationService
    {
        ...
        public AggregationService(ILogger<AggregationService> logger, IAzureTableHelper azureTableHelper, IAzureBlobHelper azureBlobHelper,
        ...
        public async Task MergeRecordOfSaleEvents(QueueMessage queueMessage)
        {
            ...
            try
            {
                ...
                var entity = await _azureTableHelper.GetEntity(message.CorrelationId, Constants.RecordOfSaleEventTableName);

                if (entity["Status"].ToString() == Status.Incomplete.ToString())
                {
                    List<string> blob = _azureBlobHelper.GetBlobNamesInFolder(Constants.RecordOfSaleEventContainerName, message.CorrelationId);
                    ...
                    if (message.RelatedEvents.All(x => blob.Contains(x)))
                    {
                        foreach (string eventId in message.RelatedEvents)
                        {
                            ...
                            string rosEvent = _azureBlobHelper.DownloadEvent(message.CorrelationId + '/' + eventId + Constants.RecordOfSaleEventFileExtension, Constants.RecordOfSaleEventContainerName);
                        ...
                        await _azureBlobHelper.UploadEvent(sapPayload.ToIndentedString(), Constants.RecordOfSaleEventContainerName, message.CorrelationId + '/' + Constants.SapXmlPayloadFileName);
                        ...
                        await _azureTableHelper.UpdateEntity(message.CorrelationId, Constants.RecordOfSaleEventTableName, new[] { new KeyValuePair<string, string>("Status", Status.Complete.ToString()) });
                    }
                    else
                    {
