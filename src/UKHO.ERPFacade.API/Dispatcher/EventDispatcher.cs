...
namespace UKHO.ERPFacade.API.Dispatcher
{
    public class EventDispatcher : IEventDispatcher
    {
        ...
        public async Task DispatchEventAsync(BaseCloudEvent baseCloudEvent)
        {
            ...
            {
                _logger.LogWarning(EventIds.InvalidEventTypeReceived.ToEventId(), "Invalid event type received. No event handler registered for event type {EventType}", eventType);
                return;
            }
        }
    }
