...
{
    public class S100ContentSapMessageBuilder : IS100ContentSapMessageBuilder
    {
        private readonly ILogger<S100ContentSapMessageBuilder> _logger;
        ...
        public S100ContentSapMessageBuilder(ILogger<S100ContentSapMessageBuilder> logger,
        ...
        public XmlDocument BuildSapMessageXml(S100EventPayload eventData, string correlationId)
        {
                ...
                foreach (var action in _sapActionConfig.Value.SapActions.Where(x => x.Product == EncCell))
                {
                    ...
                    switch (action.ActionNumber)
                    {
                        ...
                        case 4:
                            ...
                            {
                                actionNode = BuildAction(soapXml, product, unitOfSaleForReplace, action, null, replacedProduct);
                                actionItemNode.AppendChild(actionNode);
                                _logger.LogInformation(EventIds.SapActionCreated.ToEventId(), "SAP action {ActionName} created.", action.Action);
                            }
                            break;
                        ...
                        case 5:
                            ...
                            {
                                actionNode = BuildAction(soapXml, product, unitOfSaleForCoverage, action, null, additionalCoverageProduct);
                                actionItemNode.AppendChild(actionNode);
                                _logger.LogInformation(EventIds.SapActionCreated.ToEventId(), "SAP action {ActionName} created.", action.Action);
                            }
                            break;
        ...
        private XmlElement BuildAction(XmlDocument soapXml, Product product, UnitOfSale unitOfSale, SapAction action, string childCell = null, string replacedByOrAddCoverageProduct = null)
        {
