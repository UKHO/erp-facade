...
namespace UKHO.ERPFacade.Common.IO.Azure
{
    ...
    public class AzureTableReaderWriter : IAzureTableReaderWriter
    {
        ...
        private async Task<TableEntity> GetEntity(string correlationId, string tableName)
        {
        ...
        public async Task<string> GetEntityValue(string correlationId, string tableName, string entityProperty)
        {
            ...
            await foreach (var entity in entities)
            {
                if (entity.TryGetValue(entityProperty, out object fieldValue))
                {
