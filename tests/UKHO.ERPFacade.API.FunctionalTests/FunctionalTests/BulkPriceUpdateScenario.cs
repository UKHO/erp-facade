...
namespace UKHO.ERPFacade.API.FunctionalTests.FunctionalTests
{
    ...
    public class BulkPriceUpdateScenarios
    {
        ...
        [Category("DevEnvFT")]
        [Test, Order(1)]
        [TestCase("PC1_Payload.JSON", TestName = "WhenValidEventReceivedWithValidTokenandCorelationID_ThenBPUpdateReturn200OkResponse")]
        public async Task WhenValidEventReceivedWithValidTokenandCorelationID_ThenBPUpdateReturn200OkResponse(string payloadJsonFileName)
        {
            string filePath = Path.Combine(_projectDir, Config.TestConfig.PayloadFolder, "PriceChangeInputJSON", payloadJsonFileName);
            string generatedProductJsonFolder = Path.Combine(_projectDir, Config.TestConfig.GeneratedJSONProductFolder);
            var response = await _bulkPriceUpdate.PostBPUResponseAsyncForJSON(filePath, generatedProductJsonFolder, await _authToken.GetAzureADToken(false, "BulkPriceUpdate"));
            response.StatusCode.Should().Be(System.Net.HttpStatusCode.OK);
