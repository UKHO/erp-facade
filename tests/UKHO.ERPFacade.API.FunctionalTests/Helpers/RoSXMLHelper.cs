...
namespace UKHO.ERPFacade.API.FunctionalTests.Helpers
{
    ...
    {
        private static JsonInputRoSWebhookEvent s_jsonPayload;
        ...
        {
            s_jsonPayload = JsonConvert.DeserializeObject<JsonInputRoSWebhookEvent>(requestBody);
            ...
            Assert.Multiple(() =>
            {
                Assert.That(s_jsonPayload.data.correlationId, Is.EqualTo(rosXmlPayload.GUID), "GUID in xml is same a corrid as in EES JSON");
                Assert.That(VerifyPresenseOfMandatoryXMLAtrributes(rosXmlPayload).Result, Is.True);
            });

            JsonInputRoSWebhookEvent.Recordsofsale roSJsonFields = s_jsonPayload.data.recordsOfSale;
