...
{
    public static class S100XmlValidator
    {
        public static bool VerifyXmlAttributes(string generatedXmlFilePath, string actualXmlFilePath, string correlationId)
        {
            ...
            for (int i = 0; i < generatedAttributes.Count; i++)
            {
                ...
                foreach (var generatedAttribute in generatedAction.Elements())
                {
                    ...
                    if (generatedAttribute.Value != expectedAttribute?.Value)
                    {
