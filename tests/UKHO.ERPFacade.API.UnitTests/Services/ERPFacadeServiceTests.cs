...
namespace UKHO.ERPFacade.API.UnitTests.Services
{
    ...
    public class ERPFacadeServiceTests
    {
        ...
        public void WhenPriceInformationDetailIsPassed_ThenReturnsUnitsOfSalePrices()
        {
            ...
            var expectedUnitsOfSalePricesList = GetUnitsOfSalePriceList();
            ...
            result.Count.Should().Be(expectedUnitsOfSalePricesList.Count);
        }
        ...
        public void WhenValidInformationIsPassed_ThenReturnsPriceEventPayload()
        {
            ...
            var expectedUnitOfSaleUpdatedEventPayload = GetUnitsOfSaleUpdatedEventPayload();
         ...
         && call.GetArgument<IEnumerable<KeyValuePair<string, object>>>(2)!.ToDictionary(c => c.Key, c => c.Value)["{OriginalFormat}"].ToString() == "Appending UnitofSale prices to ENC event.").MustHaveHappened();

            A.CallTo(_fakeLogger).Where(call => call.Method.Name == "Log"
         && call.GetArgument<LogLevel>(0) == LogLevel.Information
         && call.GetArgument<EventId>(1) == EventIds.UnitsOfSaleUpdatedEventPayloadCreated.ToEventId()
         && call.GetArgument<IEnumerable<KeyValuePair<string, object>>>(2)!.ToDictionary(c => c.Key, c => c.Value)["{OriginalFormat}"].ToString() == "UnitofSale updated event payload created.").MustHaveHappened();

            result.Should().BeOfType<UnitOfSaleUpdatedEventPayload>();
            result.EventName.Should().Be(expectedUnitOfSaleUpdatedEventPayload.EventName);
            result.Subject.Should().Be(expectedUnitOfSaleUpdatedEventPayload.Subject);
            result.EventData.SpecVersion.Should().Be(expectedUnitOfSaleUpdatedEventPayload.EventData.SpecVersion);
            result.EventData.Type.Should().Be(expectedUnitOfSaleUpdatedEventPayload.EventData.Type);
            result.EventData.Source.Should().Be(expectedUnitOfSaleUpdatedEventPayload.EventData.Source);
            result.EventData.Id.Should().Be(expectedUnitOfSaleUpdatedEventPayload.EventData.Id);
            result.EventData.DataContentType.Should().Be(expectedUnitOfSaleUpdatedEventPayload.EventData.DataContentType);
            result.EventData.Subject.Should().Be(expectedUnitOfSaleUpdatedEventPayload.EventData.Subject);
            result.EventData._COMMENT.Should().Be("A comma separated list of products");
            result.EventData.Data._COMMENT.Should().Be("Prices for all units in event will be included, including Cancelled Cell");
            result.EventData.Data.GetType().Should().Be(expectedUnitOfSaleUpdatedEventPayload.EventData.Data.GetType());

            result.EventData.Data.UnitsOfSalePrices.Count.Should().Be(3);

            result.EventData.Data.UnitsOfSalePrices.ToList().SelectMany(u => u.Price).Count().Should().Be(4); result.EventData.Data.UnitsOfSalePrices.ToList().SelectMany(u => u.Price).Select(u => u.Standard).SelectMany(u => u.PriceDurations).Count().Should().Be(8);

            result.EventData.Data.UnitsOfSales.Count.Should().Be(3);
        }
        ...
        private UnitOfSaleUpdatedEventPayload GetUnitsOfSaleUpdatedEventPayload()
        {
            var unitsOfSalePricesList = GetUnitsOfSalePriceList();
            var existingEESJson = JsonConvert.DeserializeObject(encContentPublishedJson);

            var unitsOfSaleUpdatedEventPayload = _fakeERPFacadeService.BuildUnitsOfSaleUpdatedEventPayload(unitsOfSalePricesList, existingEESJson.ToString()!);

            return unitsOfSaleUpdatedEventPayload;
        }
    }
}
